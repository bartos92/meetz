        ...
        <div :class='{noDisplay: hideDate}'>
          ...
          <div>
            <div class="week" v-for="(week, weekIndex) in weeks">
              <span class="port" v-for="(day, dayIndex) in week" v-on:click='setDay(weekIndex*7 + dayIndex, day)' @dblclick="setDate" :class='{activePort: (weekIndex*7 + dayIndex) === activePort}'>
                {{day}}
              </span>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    makeDateObject (val) {
      return moment(val, 'DD-MM-YYYY HH:mm:ss').toDate();
    }
  },
  created () {
  	if (this.value) {
